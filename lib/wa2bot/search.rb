...
module Wa2Bot
  module Search
    ...
    def save_searched_tweets
      ...
      unless File.exist? File.dirname(HISTORY_FILE)
    ...
    def save_retweet_id(ids)
      ...
      unless File.exist? File.dirname(RETWEET_FILE)
        FileUtils.mkdir_p File.dirname(RETWEET_FILE)
      end
    ...
    def remove_duplication(tweets)
      texts, ids = [], []
      tweets.each.with_object([]) do |tweet, uniques|
